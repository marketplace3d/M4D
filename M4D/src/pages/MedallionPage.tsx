...
export default function MedallionPage() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:C.bg, color:C.text, fontFamily:C.mono, overflow:'hidden' }}>
      ...
      {/* ── 4-COLUMN BODY ── */}
      <div style={{ flex:1, display:'grid', gridTemplateColumns:'200px 200px 1fr 260px', gap:0, overflow:'hidden' }}>

        {/* COL 1 — Architecture */}
        <div style={{ borderRight:`1px solid ${C.border}`, overflow:'auto', padding:'8px 8px' }}>
          {colHead('⬡ ARCHITECTURE', C.gold)}
          {ARCH.map((b,i) => chip(b, detailSrc==='arch' && detail?.id===b.id, ()=>selectArch(b)))}
        ...
        <div style={{ borderRight:`1px solid ${C.border}`, overflow:'auto', padding:'8px 8px' }}>
          ...
          {DISCOVERY.map((b,i) => chip(b, detailSrc==='disc' && detail?.id===b.id, ()=>selectDisc(b)))}
          <div style={{marginTop:8,fontSize:8,color:C.faint,lineHeight:1.6}}>
        ...
        <div style={{ overflow:'auto', padding:'8px 10px', display:'flex', flexDirection:'column', gap:10 }}>
          ...
          <div style={{ marginTop:'auto', padding:'8px 8px', background:C.bg1, borderRadius:4, fontSize:8, color:C.faint, lineHeight:1.7, border:`1px solid ${C.border}` }}>
            ...
            <span style={{color:C.purple}}>→ #star</span> for full opts lab
          </div>
        </div>
      </div>
    </div>
