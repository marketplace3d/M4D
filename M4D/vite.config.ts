...
export default defineConfig(({ mode }) => {
  // Load keys from M4D/.env.local + legacy M1D/.env.local
  const envM4d = loadEnv(mode, __dirname, '')
  const envM1d = loadEnv(mode, path.resolve(repoRoot, 'M1D'), '')
  const envPwa = loadEnv(mode, path.resolve(repoRoot, 'pwa'), '')

  const polygonKey =
    envM4d.VITE_POLYGON_IO_KEY || envM4d.VITE_POLYGON_API_KEY ||
    envM1d.VITE_POLYGON_IO_KEY || envM1d.VITE_POLYGON_API_KEY ||
