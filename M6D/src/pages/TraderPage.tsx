...
function SignalBar({ sig, info }: { sig: string; info: any }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
      <span style={{ width: 88, fontSize: 8, color: ic > 0 ? C.text : C.muted, fontFamily: 'monospace' }}>{sig}</span>
      <span style={{ width: 42, fontSize: 7, color: regC }}>{info.home_regime?.slice(0,6)}</span>
      <div style={{ width: 40, height: 4, background: '#0d1f2e', borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ width: `${bar * 100}%`, height: '100%', background: ic > 0 ? C.HEALTHY : C.RETIRE, borderRadius: 2 }} />
      </div>
      <span style={{ width: 52, fontSize: 8, color: stC, fontFamily: 'monospace' }}>
        {ic > 0 ? '+' : ''}{ic.toFixed(4)}
      </span>
      <Pill label={info.status} col={stC} />
    </div>
  );
}
...
function SignalsTab({ ic, wf, onRun }: { ic: any; wf: any; onRun: (ep: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
      ...
      {/* Signal lifecycle */}
      <Card title="L2-5 · SIGNAL LIBRARY · REGIME IC" w={380} accent="#3ae87a">
        <div style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
          {[
            { l: 'RETIRE', n: retire.length, c: C.RETIRE },
            { l: 'SLOW',   n: ic?.slow?.length ?? 0,    c: C.SLOW },
            { l: 'WATCH',  n: ic?.watch?.length ?? 0,   c: C.WATCH },
            { l: 'HEALTHY',n: ic?.healthy?.length ?? 0, c: C.HEALTHY },
          ].map(b => (
            <div key={b.l}>
      ...
      <Card title="WALK-FORWARD · 41 FOLDS · REGIME BREAKDOWN" w={260} accent="#3ae87a">
        {wf ? (
          <>
            <Row k="OOS Sharpe mean" v={(wf.summary?.oos_sharpe?.mean ?? 0).toFixed(3)} vc={(wf.summary?.oos_sharpe?.mean ?? 0) > 0 ? C.HEALTHY : C.RETIRE} />
            ...
            <Row k="OOS % positive" v={`${((wf.summary?.oos_sharpe?.pct_positive ?? 0) * 100).toFixed(0)}%`} />
            ...
            {['TRENDING', 'BREAKOUT', 'RANGING', 'RISK-OFF', 'MIXED'].map(r => {
              ...
              return (
                <div key={r} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3, alignItems: 'center' }}>
                  <span style={{ fontSize: 8, color: (C as any)[r] ?? C.muted }}>{r}</span>
                  <span style={{ fontSize: 8, color: c, fontFamily: 'monospace' }}>
                    {sh > 0 ? '+' : ''}{sh.toFixed(1)} · {pp.toFixed(0)}%pos
                  </span>
                </div>
              );
            })}
...
function RoutingTab({ ens, gate, cross, pca, onRun }: { ens: any; gate: any; cross: any; pca: any; onRun: (ep: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
      ...
      <Card title="L8-11 · ENSEMBLE ROUTING · OOS BRANCHES" w={260} accent="#b07aff">
        <div style={{ marginBottom: 6 }}>
          {[
            { l: 'Equal weight',    d: eq,   c: C.muted, best: false },
            { l: 'Sharpe-weighted', d: sw,   c: C.PADAWAN, best: false },
            { l: 'Hard routed',     d: hard, c: C.WATCH, best: false },
            { l: 'SOFT routed ★',  d: soft, c: C.HEALTHY, best: true },
          ].map(({ l, d, c, best }) => (
            <div key={l} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              marginBottom: 4, padding: '2px 4px', borderRadius: 3,
              background: best ? '#04140a' : 'transparent',
              border: best ? `1px solid #0d2e18` : 'none',
            }}>
              <span style={{ fontSize: 8, color: c }}>{l}</span>
        ...
        <Row k="Hard routing delta" v={ens?.hard_routing_delta != null ? `${ens.hard_routing_delta > 0 ? '+' : ''}${ens.hard_routing_delta.toFixed(3)}` : '—'} vc={ens?.hard_routing_delta > 0 ? C.HEALTHY : C.RETIRE} />
...
function BrokerTab({ ibkr, onRun }: { ibkr: any; onRun: (ep: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
      ...
      <Card title="IBKR · TWS LIVE CONNECTION" w={250} accent={connected ? C.HEALTHY : C.RETIRE}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
          <div style={{
            ...
            background: connected ? C.HEALTHY : C.RETIRE,
            boxShadow: connected ? `0 0 8px ${C.HEALTHY}` : 'none',
            flexShrink: 0,
          }} />
          <span style={{ fontSize: 12, fontWeight: 700, color: connected ? C.HEALTHY : C.RETIRE, fontFamily: 'Barlow Condensed, sans-serif', letterSpacing: 2 }}>
      ...
      {/* Recent trades */}
      <Card title="RECENT TRADES · IBKR" w={220} accent={C.PADAWAN}>
        {trades.length > 0
          ? trades.slice(0, 8).map((t: any, i: number) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                <span style={{ fontSize: 7.5, color: C.text }}>{t.symbol}</span>
                <span style={{ fontSize: 7.5, color: t.action === 'ENTRY' ? C.HEALTHY : C.WATCH }}>
                  {t.action}
                </span>
                <span style={{ fontSize: 7.5, color: t.side === 'buy' ? C.HEALTHY : C.RETIRE, fontFamily: 'monospace' }}>
                  {t.side}
                </span>
                {t.pnl_usd != null && (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: C.bg, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {[
          ...
          { l: 'F&G',    v: fngVal ?? '—', c: fngVal != null ? (fngVal <= 44 ? C.HEALTHY : fngVal >= 56 ? C.RETIRE : C.muted) : C.muted },
          { l: 'IBKR',   v: ibkrConn ? 'LIVE' : 'OFF',  c: ibkrConn ? C.HEALTHY : C.RETIRE },
          { l: 'MRT',    v: (crossData as any)?.mrt_vol_label ?? '—', c: C.text },
          { l: 'TICK',   v: tick, c: C.dim },
        ].map(k => (
          <span key={k.l} style={{ fontSize: 9, flexShrink: 0 }}>
