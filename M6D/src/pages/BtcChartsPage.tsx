...
const CHART_STRIP_ID: ChartStripId = 'btc';

export default function BtcChartsPage() {
  const TOP_STOCKS = ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'DOGE', 'ADA', 'AVAX', 'DOT', 'MATIC', 'LINK', 'UNI'] as const;
    ...
    <div className="tv-lw-page">
      ...
      <div className="tv-lw-control-strip">
        <div className="tv-lw-masters-row" role="group" aria-label="Chart overlays">
          ...
          <div className="tv-lw-masters-seg" role="group" aria-label="Binance DOM">
            <button
              type="button"
              className={showDom ? 'tv-lw-pill tv-lw-pill--on' : 'tv-lw-pill'}
              onClick={() => setShowDom((v) => !v)}
              title="DOM — Binance 20-level full depth heatmap + trade bubbles (free · no auth · 100ms)"
            >
              DOM
            </button>
          </div>
      ...
      {showDom && <BinanceObiPanel snap={bnbSnap} sym={sym} />}
