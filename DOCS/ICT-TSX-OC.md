...
**Code entry point:** `pwa/src/lib/ictLiquiditySynthesis.ts` → `computeIctSynthesis(bars, { asset, tf, dailyBars?, compact? })`

- **Gravity distance:** `proximityTaper(proxPct)` treats `proxPct` as **percent of price** (e.g. `2.5` = 2.5%), so far levels are penalized vs the old `1/(1+proxPct)` bug.
- **Primary stop class:** `ictClassForPrimaryStop` matches **by `mm.nextStopKind` first** (aligns LTF/HTF), then **price within 0.1×ATR** (wider than 0.02), then `classifyLevel(kind)`.
- **Council tokens:** set **`compact: true`** → top **3** levels, **2** Oracle context lines, `mode=COMPACT` in the header. Result includes **`councilCompact`**.
...
3. **Direction priority** (`direction`): `BULL` | `BEAR` | `NEUTRAL` with **rawStrength** (base) and **strength** (after MM phase adjusters). **MANIPULATION** applies a **0.68×** discount so Council/TSX do not treat a stop-raid like a full **DISPLACEMENT** leg; **DISPLACEMENT** gets a small boost. See `directionFrom` in `ictLiquiditySynthesis.ts`.
...
| 2026-04-25 | `proximityTaper` gravity fix; `ictClassForPrimaryStop` kind-first + 0.1 ATR; `compact` + `councilCompact`; M4D `d.ts` promoted ICT types |
